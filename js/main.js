/* =====================================================
   LUNCHMATE — main.js  (Netflix Theme)
   ===================================================== */

const state = {
  page: 'feed', feedFilter: 'ทั้งหมด',
  searchMeal: '', searchFood: '',
  selectedFood: '', myJoined: [],
  myName: 'ผู้ใช้', myAvatar: '😊', posts: [],
};
...
function loadState() {
  try {
    const s = JSON.parse(localStorage.getItem('lm3_state') || 'null');
    if (s) {
...
function saveState() {
  localStorage.setItem('lm3_state', JSON.stringify({
    myJoined: state.myJoined, myName: state.myName,
    myAvatar: state.myAvatar, posts: state.posts,
  }));
}
...
function seedPosts() {
  state.posts = [
    ...
    { id:1003, name:'บอล นักกิน', avatar:'🧑',
      ...
      note:'ไปกินหลังเลิกเรียน เดินทางด้วยกันได้ นั่ง BTS สาย Sukhumvit ไปด้วยกัน',
...
function goTo(page) {
  ...
  document.getElementById('nav-avatar-emoji').textContent = state.myAvatar;
...
function cardHTML(p, idx) {
  ...
  const btnTxt= full&&!iJoin?'😢 เต็มแล้ว' : iJoin?'✅ เข้าร่วม':'▶ เข้าร่วม';
  ...
  <div class="post-card" style="animation-delay:${delay}s">
    ...
    <div class="card-body">
      <div class="card-rest"><span class="r-emoji">${p.foodEmoji}</span>${p.restaurant}</div>
...
function renderFeed() {
  ...
  if(trenEl) trenEl.innerHTML=sorted.map((r,i)=>`
    <div class="trending-item">
      <span class="tr-rank">${i+1}</span>
      <div><div class="tr-name">${r[0]}</div><div class="tr-count">${r[1]} คนสนใจ</div></div>
    </div>`).join('');
  ...
  if(!list.length){
    container.innerHTML=`<div class="empty"><div class="empty-em">🎬</div>
      <div class="empty-t">ไม่มีเนื้อหาในมื้อนี้</div><p>เป็นคนแรกที่โพสต์!</p></div>`;
    return;
  }
...
function renderSearch() {
  ...
  let list=[...state.posts].reverse().filter(p=>{
    ...
    const mM=!state.searchMeal||p.meal===state.searchMeal;
    const mF=!state.searchFood||p.foodEmoji===state.searchFood;
    return mQ&&mM&&mF;
  });
  const c=document.getElementById('search-count');
  if(c) c.textContent=`พบ ${list.length} โพสต์`;
  const container=document.getElementById('search-results');
  if(!list.length){
    container.innerHTML=`<div class="empty" style="padding:40px 20px"><div class="empty-em">🔍</div>
      <div class="empty-t">ไม่พบผลลัพธ์</div><p>ลองเปลี่ยนคำค้นหา</p></div>`;
    return;
  }
  container.innerHTML=list.map((p,i)=>cardHTML(p,i)).join('');
}
...
function joinPost(id){
  ...
  state.myJoined.push(id);saveState();renderCurrentPage();
...
function sendComment(id){
  ...
  p.comments.push(c);inp.value='';saveState();
  const list=document.getElementById('clist-'+id);
  if(list){
    const div=document.createElement('div');
    div.className='comment-item';div.style.animation='fadeUp .3s ease';
...
function submitPost(){
  ...
  const np={
    id:Date.now(),name:state.myName,avatar:state.myAvatar,
    restaurant,foodEmoji:state.selectedFood||'🍽️',
    area:area||'',meal:meal||'กลางวัน',time,slots,
    joined:1,members:[state.myName],
    note:note||'ไม่มีรายละเอียดเพิ่มเติม',
    createdAt:Date.now(),comments:[]
  };
  state.posts.push(np);state.myJoined.push(np.id);saveState();
  ['f-restaurant','f-time','f-note','f-area'].forEach(id=>{
    const el=document.getElementById(id);if(el)el.value='';
  });
  state.selectedFood='';
  document.querySelectorAll('.food-btn').forEach(b=>b.classList.remove('sel'));
  goTo('feed');toast('✅ โพสต์สำเร็จ! รอเพื่อนมาร่วมกันนะ');
}
...
function renderProfile(){
  ...
  if(!myPosts.length){
    listEl.innerHTML=`<div class="empty"><div class="empty-em">🎬</div>
  ...
  listEl.innerHTML=[...myPosts].reverse().map(p=>`
    <div class="my-post-card">
      ...
      <div class="my-post-meta">
        <span>🕐 ${p.time} น.</span><span>👥 ${p.joined}/${p.slots} คน</span>
