...
function loadState(){
  try{
    ...
    if(s){ state.myJoined=s.myJoined||[]; state.myName=s.myName||'นักศึกษา SPU'; state.myAvatar=s.myAvatar||'🎓'; state.posts=s.posts||[]; }
  }catch(e){}
  if(!state.posts.length) seed();
}
function save(){
  localStorage.setItem('spu_lm',JSON.stringify({myJoined:state.myJoined,myName:state.myName,myAvatar:state.myAvatar,posts:state.posts}));
}
function seed(){
  state.posts=[
