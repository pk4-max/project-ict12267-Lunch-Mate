...
function loadState(){
  try{
    ...
    if(s){
      ...
      state.myName=s.myName||'นักศึกษา SPU';
...
function updateMobileMenu(page){
  document.querySelectorAll('.mobile-nav-link').forEach(n=>n.classList.toggle('active', n.dataset.page===page));
...
/* ── BANNER CAROUSEL ──────────────────────────────── */
const BANNER = { cur:0, total:0, timer:null, prog:null, progRaf:null, progStart:0, INTERVAL:5000, dragging:false, dragStartX:0 };
...
function initBanner(){
  ...
  BANNER.total = slides.length;

  // Apply gradient CSS vars from data attributes
  slides.forEach(sl => {
    sl.style.setProperty('--bs-from', sl.dataset.colorFrom || '#006633');
    sl.style.setProperty('--bs-to',   sl.dataset.colorTo   || '#00a84a');
  });

  // Build dots
  dotsEl.innerHTML = '';
  for(let i=0;i<BANNER.total;i++){
    const d = document.createElement('button');
    d.className = 'bdot' + (i===0?' active':'');
    d.onclick = ()=>bannerGo(i);
    dotsEl.appendChild(d);
  }

  // Touch/drag support
  const wrap = document.getElementById('banner-wrap');
  wrap.addEventListener('mousedown',  e=>{ BANNER.dragging=true; BANNER.dragStartX=e.clientX; });
  wrap.addEventListener('mousemove',  e=>{ if(!BANNER.dragging) return; e.preventDefault(); });
  wrap.addEventListener('mouseup',    e=>{ if(!BANNER.dragging) return; BANNER.dragging=false; const dx=e.clientX-BANNER.dragStartX; if(Math.abs(dx)>40) bannerStep(dx<0?1:-1); });
  wrap.addEventListener('mouseleave', e=>{ BANNER.dragging=false; });
  wrap.addEventListener('touchstart', e=>{ BANNER.dragStartX=e.touches[0].clientX; }, {passive:true});
  wrap.addEventListener('touchend',   e=>{ const dx=e.changedTouches[0].clientX-BANNER.dragStartX; if(Math.abs(dx)>40) bannerStep(dx<0?1:-1); }, {passive:true});
...
function bannerGo(idx){
  ...
  BANNER.cur = (idx + BANNER.total) % BANNER.total;

  // Slide track
  track.style.transform = `translateX(-${BANNER.cur * 100}%)`;

  // Re-trigger animation on visible slide
  const slides = track.querySelectorAll('.banner-slide');
  slides.forEach((s,i)=>{
    const c = s.querySelector('.bs-content');
    const v = s.querySelector('.bs-visual');
    if(i === BANNER.cur){
      if(c){ c.style.animation='none'; void c.offsetWidth; c.style.animation='bsIn .42s ease both'; }
      if(v){ v.style.animation='none'; void v.offsetWidth; v.style.animation='bsFloat 3.5s ease-in-out infinite'; }
    }
  });

  // Dots
  if(dotsEl) dotsEl.querySelectorAll('.bdot').forEach((d,i)=>d.classList.toggle('active',i===BANNER.cur));

  // Restart auto-timer & progress
  clearTimeout(BANNER.timer);
  startBannerProgress();
  BANNER.timer = setTimeout(()=>bannerStep(1), BANNER.INTERVAL);
}

function bannerStep(dir){
  bannerGo(BANNER.cur + dir);
}

function startBannerProgress(){
  const bar = document.getElementById('banner-progress');
  if(!bar) return;
  cancelAnimationFrame(BANNER.progRaf);
  bar.style.transition = 'none';
  bar.style.width = '0%';
  BANNER.progStart = performance.now();
  function tick(now){
    const pct = Math.min(100, (now - BANNER.progStart) / BANNER.INTERVAL * 100);
    bar.style.width = pct + '%';
    if(pct < 100) BANNER.progRaf = requestAnimationFrame(tick);
  }
  BANNER.progRaf = requestAnimationFrame(tick);
}

// Keep old name for compatibility
function startCarousel(){ initBanner(); }
...
function showLoginModal(){
  const el = document.getElementById('lp-login-sec');
  if(el) el.scrollIntoView({behavior:'smooth', block:'center'});
  setTimeout(()=>{ document.getElementById('login-sid')?.focus(); }, 500);
}
...
function updateLoginUI(){
  ...
  const mobileLbl = document.getElementById('mobile-login-lbl');
  if(btn){
    if(state.loggedIn){
  ...
  if(mobileBtn && mobileLbl){
    mobileLbl.textContent = state.loggedIn ? 'ออกจากระบบ' : 'เข้าสู่ระบบ';
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(localStorage.getItem('spu_lm_ver') !== APP_VER){
    localStorage.removeItem('spu_lm');
    localStorage.setItem('spu_lm_ver', APP_VER);
  }
  loadState();
  updateLoginUI();
  initBanner();
